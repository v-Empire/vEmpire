...
const MasterChefVemp = contract.fromArtifact('MasterChefBEP20Vemp');
...
describe('MasterChefVemp', function () {
    ...
    beforeEach(async function () {
        ...
        this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
        await this.chef.initialize(this.xVemp.address, ownerAddress, "1000", "0", "1000", {from: ownerAddress, gas: 8000000});
    ...
    describe("With ERC/LP token added to the field", function () {
        ...
        it("should give out xVemp only after farming time", async function () {
            // 100 per block farming rate starting at block 100 with bonus until block 1000
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "7200", "7400", {from: ownerAddress, gas: 8000000});
        ...
        it("should not distribute Vemp if no one deposit", async function () {
            ...
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "7400", "7600", {from: ownerAddress, gas: 8000000});
            ...
            await time.advanceBlockTo("7499")
            expect(await this.vemp.balanceOf(this.chef.address)).to.be.bignumber.equal(new BN(100000000))
            await time.advanceBlockTo("7504")
            expect(await this.vemp.balanceOf(this.chef.address)).to.be.bignumber.equal(new BN(100000000))
            await time.advanceBlockTo("7509")
            ...
            await time.advanceBlockTo("7519")
        ...
        it("should distribute xVemp properly for each staker", async function () {
            ...
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "7600", "7700", {from: ownerAddress, gas: 8000000});
            ...
            await time.advanceBlockTo("7629")
        ...
        it("should stop giving bonus xVemp after the bonus period ends", async function () {
            ...
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "7800", "7900", {from: ownerAddress, gas: 8000000});
            ...
            await time.advanceBlockTo("7889")
            await this.chef.deposit(0, "10", { from: userAddress1 })
            
            await time.advanceBlockTo("7905")
        ...
        it("Transfer Ownership", async function () {
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "7900", "8000", {from: ownerAddress, gas: 8000000});
        ...
        it("Distribute rewards after changing end block", async function () {
            this.chef = await MasterChefVemp.new({from: ownerAddress, gas: 8000000});
            await this.chef.initialize(this.xVemp.address, ownerAddress, "100", "8000", "8100", {from: ownerAddress, gas: 8000000});

            await this.chef.updateEndBlock(200, {from: ownerAddress, gas: 8000000});
            expect(await this.chef.bonusEndBlock()).to.be.bignumber.equal(new BN(200))
        })
    })
})