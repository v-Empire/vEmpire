...
describe('MasterChefLP', function () {
    ...
    beforeEach(async function () {
        ...
        await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "1000", "0", 100, 120, { from: ownerAddress, gas: 8000000 });
    ...
    describe("updateVempLockAmount", function () {
        it("If non-admin try to update updateVempLockAmount", async function () {
            await expectRevert(this.chef.updateVempLockAmount(101, { from: userAddress1, gas: 8000000 }), "Ownable: caller is not the owner.");
        })

        it("If admin try to update updateVempLockAmount", async function () {
            await this.chef.updateVempLockAmount(101, { from: ownerAddress, gas: 8000000 });
            expect(await this.chef.vempLockAmount()).to.be.bignumber.equal(new BN(101))
        })
    })
    ...
    describe("lock", function () {
        it("Lock vemp Tokens", async function () {
            ...
            await this.chef.lock({ from: ownerAddress, gas: 8000000 });
    ...
    describe('Only admin can stop to withdraw tokens', function () {
        it("allow withdraw staked tokens with lock periode", async function () {
            ...
            await this.chef.lock({ from: userAddress2, gas: 8000000 });
    ...
    describe("With ERC/LP token added to the field", function () {
        it("should give out vemp only after farming time", async function () {
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "13700", 100, 120, { from: ownerAddress, gas: 8000000 });
        ...
        it("should not distribute vemp if no one deposit", async function () {
            ...
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "13800", 100, 120, { from: ownerAddress, gas: 8000000 });
        ...
        it("should distribute vemp properly for each staker", async function () {
            ...
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "14100", 100, 120, { from: ownerAddress, gas: 8000000 });
        ...
        it("Transfer Ownership", async function () {
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "14660", 100, 120, { from: ownerAddress, gas: 8000000 });
    ...
    describe("Access LP Tokens by Admin", function () {
        it("Failed if any user(not admin) want to access LP tokens", async function () {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "15200", 100,120, { from: ownerAddress, gas: 8000000 });
        ...
        it("Failed if any admin want to access LP tokens more then locked amount", async function () {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "15700", 100, 120, { from: ownerAddress, gas: 8000000 });
        ...
        it("Update All variables on stake and access LP tokens", async function () {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "16200", 100, 120, { from: ownerAddress, gas: 8000000 });
    ...
    describe("Re-Distribution of LP tokens to all stake holders in rewards", function () {
        it("Re-Distribute LP tokens by Admin", async function () {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "16800", 100, 120, { from: ownerAddress, gas: 8000000 });
    ...
    describe("Claim LP Reward tokens", function () {
        it("Claim LP tokens by stakers", async function () {
            // 100 per block farming rate starting at block 300 with bonus until block 1000
            this.chef = await MasterChefLP.new({ from: ownerAddress, gas: 8000000 });
            await this.chef.initialize(this.vemp.address, this.LP.address, ownerAddress, "100", "17300", 100, 120, { from: ownerAddress, gas: 8000000 });
